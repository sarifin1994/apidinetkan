...
{
    protected $signature = 'invoice:fixed-date';
    protected $description = 'Generate invoices for fixed-date PPPoE users before due date';
...
//             ->where('is_dinetkan','0')
    ...
    protected function processUser($user)
    {
        ...
        foreach ($user->c_pppoe_fixed as $pppoe) {
            ...
            $targetDatex = Carbon::parse($targetDate)->format('d');
            $todayx = Carbon::parse($today)->format('d');

            if ($targetDatex !== $todayx) {
                continue;
            }

            // check inv unpaid
            $unpaid = Invoice::query()->where('id_pelanggan', $pppoe->id)->where('status', 'unpaid')->get();
            if(count($unpaid) == 2){
                continue;
            }
