...
class SuspendPppoeUser_debug extends Command
{
    protected $signature = 'pppoe:suspend:debug';
    ...
    protected function processUser($user, $sshUser, $sshHost, array $sshOptions, $radPort, $radRetries)
    {
        ...
        foreach ($user->c_pppoe1 as $pppoe) {
            if($pppoe->username != 'udansungalah@dinetkan.com'){
                continue;
            }
            Log::info('masuk trap => '.$pppoe->username);
        ...
        foreach ($targets as $nas) {
            ...
            try {
                ...
                if ($process->successful()) {
                    ...
                    // ]);
                    Log::info('masul error');
                    Log::info($process->errorOutput());
                    Log::info($process->exitCode());
                }
            } catch (\Exception $e) {
