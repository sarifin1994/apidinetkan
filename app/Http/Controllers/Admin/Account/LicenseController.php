...
final class LicenseController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('accounts.licensing.index', compact('licenses'));
    }
    ...
    public function order(Request $request, string $id,$couponCode = "")
    {
        ...
        if(count($invoices) > 0){
            return redirect()->route('admin.account.invoice.index')->with('warning', 'You have unpaid invoice ');
        }
        // Disallow downgrade
        ...
        return view('accounts.licensing.order', [
            'license' => $license,
    ...
    public function placeOrder(Request $request, int $licenseId)
    {
        ...
        return redirect()->route('admin.invoice', $invoice->no_invoice);
    }
    ...
    public function invoice(Request $request, string $no_invoice)
    {
        ...
        return view('accounts.licensing.invoice', [
...
//        $priceDataNew = $this->adminPaymentService->get_promo_coupon($invoice->admin,$invoice->itemable,$priceData, $invoice->couponname);
        ...
        if ($priceData->total === 0) {
            $this->adminPaymentService->markInvoiceAsPaid($invoice);
            return response()->json(['url' => route('admin.account.licensing.thank-you', $invoice->no_invoice)]);
        }
    ...
    public function thankYou(string $invoiceNo)
    {
        ...
        return view('accounts.licensing.thank-you', compact('invoice'));
    }
}
