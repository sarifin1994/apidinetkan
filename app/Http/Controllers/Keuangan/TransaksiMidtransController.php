...
class TransaksiMidtransController extends Controller
{
    public function index()
    {
        ...
        $incomeYear = Transaksi::where('shortname', multi_auth()->shortname)
            ...
            // ->sum('nominal');
            $totalWithdraw = MidtransWithdraw::where('shortname', multi_auth()->shortname)
            ->sum('nominal');
        ...
        if (request()->ajax()) {
            ...
            // } else {
                $transaksi = Transaksi::query()->where('shortname', multi_auth()->shortname)->where('created_by','midtrans');
            // }
            return DataTables::of($transaksi)
                ->addIndexColumn()
                ->toJson();
        }
        return view('backend.keuangan.midtrans.index_new', compact('incomeMonth','incomeLastMonth','incomeYear','totalSaldo','midtrans'));
    }

    public function index_withdraw(){
        if (request()->ajax()) {
            $withdraw = MidtransWithdraw::query()->where('shortname', multi_auth()->shortname);
    ...
    public function withdraw(Request $request){
        if($request->nominal !== '0'){
            $witdhraw = MidtransWithdraw::create([
                'shortname' => multi_auth()->shortname,
                'tanggal' => Carbon::now(),
                'id_penarikan' => 'WD-' . now()->format('ymd') . rand(100, 999),
