    ...
    public function bayar()
    {
        ...
        if (!$invoice) {
            ...
            $transaction_details = [
                ...
                'gross_amount' => (int) $invoice->price + (int) $midtrans->admin_fee + (int) $amount_ppn - (int) $amount_discount, // no decimal allowed for creditcard
            ];
                    ...
                    if ($transaction == 'capture') {
                        ...
                        if ($user->order !== null) {
                            $user_update->update([
                                ...
                                'next_due' => Carbon::parse($user->next_due)->addMonth()->toDateString(),
                                'order' => null,
                                'order_status' => 'paid',
                            ]);
                        } else {
...
Pembayaran lisensi *' .
                            ...
                            $user->order_number .
                            ' senilai *Rp ' .
                            number_format($total_midtrans, 0, ',', '.') .
                            '* telah kami terima.
Silakan login ke dashboard '.env('APP_URL').' untuk mengecek status akunmu<br>
