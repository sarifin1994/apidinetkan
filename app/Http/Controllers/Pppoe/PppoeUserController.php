...
class PppoeUserController extends Controller
{
    public function index()
    {
        ...
        $odps = Odp::where('shortname', multi_auth()->shortname)->get();
        ...
        if (request()->ajax()) {
            ...
            return DataTables::of($users)
                ...
                ->addColumn('checkbox', function ($row) {
                    return '<input type="checkbox" class="row-cb form-check-input" id="checkbox_row' . $row->id . '" value="' . $row->id . '"/>';
                })
...
//                    return $row->rprofile->name;
        ...
        $provinces = Province::query()
            ->orderBy('name', 'asc')
            ->get();
        return view('backend.pppoe.user.index_new',
            compact('areas',
                'nas',
                'profiles',
                'mitras',
