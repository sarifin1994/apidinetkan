...
<body>
  @php
    $nickname = \App\Models\Company::where('group_id', auth()->user()->id_group)
        ->select('nickname')
        ->first()->nickname;
    $i = 1;
  @endphp
    ...
    @php
      ...
      if ($item->rprofile->validity === 'Unlimited') {
          ...
          if ($item->rprofile->validity >= 2592000) {
              ...
              $validity = Carbon\CarbonInterval::seconds((int) $item->rprofile->validity)->cascade()->forHumans();
          }
      }

      if ($item->rprofile->uptime === 'Unlimited') {
          $uptime = 'Unlimited';
      } else {
          if ($item->rprofile->uptime >= 2592000) {
              $uptime = Carbon\CarbonInterval::seconds((int) $item->rprofile->uptime)
                  ->cascade()
                  ->forHumans([
                      'skip' => ['day', 'hour', 'minute', 'second'],
                  ]);
          } else {
              $uptime = Carbon\CarbonInterval::seconds((int) $item->rprofile->uptime)->cascade()->forHumans();
          }
      }
              ...
              <tbody>
                <tr>
                  <td style="width:75px">
                    ...
                    <h1 style="margin:-18px 0 0 0;font-size:14px;color:#fe0e01">{{ $nickname }}</h1>
                  </td>
                  <td style="width:115px">
                    <div style="float:right;margin-top:13px;margin-right:0px;width:5%;text-align:right;font-size:7px;">
                    </div>
          ...
          <td style="color:#666;border-collapse: collapse;" valign="top">
                  ...
                  <tr>
                    <td style="width:100px;text-align:left;">
                      <div style="text-align:left;color:#222;font-size:7px;margin:0px;padding:2.5px;">
                        Validity : {{ Illuminate\Support\Str::title($validity) }}
                        Uptime : {{ Illuminate\Support\Str::title($uptime) }}
                      </div>
                    </td>
                    <td style="width:100px;text-align:left;">
                      <div style="text-align:left;color:#222;font-size:7px;margin:0px;padding:2.5px;">
                  ...
                  <tr>
                    ...
                    <td style="width:100px;text-align:right;">
                      <div style="text-align:right;color:#222;font-size:7px;margin:0px;padding:2.5px;">
                        Validity : {{ Illuminate\Support\Str::title($validity) }}<br>
                        Uptime : {{ Illuminate\Support\Str::title($uptime) }}<br>
                        Kuota : {{ $quota }}<br>
                      </div>
                    </td>
                  </tr>
            ...
            <table style="width:100%;border-collapse: collapse;">
              <tbody>
                ...
                <tr>
                  <td class="warnalist" style="color:#666;padding:0px;background:#fe0e01" valign="top" colspan="1">
                  ...
                  <td class="warnalist" style="color:#666;padding:0px;background:#fe0e01" valign="top" colspan="1">
                    <div style="text-align:right;color:#fff;font-size:6px;font-weight:bold;margin:0px;padding:2.5px;">
                      <b>[{{ $i++ }}]</b>
                    </div>
                  </td>
                </tr>
