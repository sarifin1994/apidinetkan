                    ...
                    <div class="card-body">
                        <form id="adminForm" method="POST">
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select class="form-control" id="dinetkan_user_id" name="dinetkan_user_id" required>
                                        @foreach ($resellers as $row)
                                            <option value="{{ $row->dinetkan_user_id }}">{{ $row->first_name }}
                                                {{ $row->last_name }} -
                                ...
                                <div class="col-md-6 mb-3">
                                    <input type="hidden" name="id_mitra" id="id_mitra_hidden">
                                    <label for="id_mitra" class="form-label">Partnership</label>
                                    <select class="form-control" id="id_mitra" required>
                                        <option value="">-- Partnership --</option>
                                        @foreach ($mitras as $row)
                                            <option value="{{ $row->id_mitra }}">{{ $row->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="col-md-4 mb-3">
                                    ...
                                    <select class="form-control" id="statuses" name="statuses" required>
                                        @foreach ($statuses as $key => $val)
                                            <option value="{{ $key }}" {{ $key == $progress ? 'selected' : '' }}>
                                                {{ $val }}</option>
                                        @endforeach
                                    </select>
                                </div>
...
@push('scripts')
        ...
        $(document).ready(function() {
            $('#id_mitra').on('change', function() { 
                ...
                if(id_mitra){
                    ...
                    $.ajax({
                        url: baseurl + `/dinetkan/invoice_dinetkan/by_id_mitra`,
                        type: 'POST',
                        data: {
    ...
    <script>
        ...
        $('#dinetkan_user_id').on('change', function() { 
            const id = $(this).val();
            $.ajax({
                url: baseurl + `/dinetkan/invoice_dinetkan/get_user/` + id,
                type: 'GET',
                dataType: "json",
                success: function(data) {
                    if(data.created_by != '' && data.created_by != null){
                        $('#id_mitra').val(data.created_by).trigger('change');
                        $('#id_mitra').prop('disabled', true);
                    }else{
                        $('#id_mitra').val("").trigger('change');
                        $('#id_mitra').prop('disabled', false);
                    }
                }
            });
        });
