      ...
      <div class="row">
        ...
        <div class="col-sm-6 pe-0">
          <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">Master Server</li>
          </ol>
        </div>
      </div>
    </div>
          ...
          <div class="card-body">
            <div class="bg-light-primary b-r-15">
              <div class="upcoming-box d-flex align-items-center justify-content-between px-4">
                ...
                <button type="button" class="btn btn-primary" id="openCreateModal">Add Server</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <form id="couponForm" method="POST">
          ...
          <div class="modal-body">
            ...
            <div class="mb-3">
                ...
                @if($masterGroupServer)
                    @foreach($masterGroupServer as $row)
            ...
            <div class="mb-3">
                <label for="is_notif" class="form-label">Notification Status</label>
                <select class="form-select" id="is_notif" name="is_notif" required>
  ...
  <script>
    $(document).ready(function() {
      ...
      $('#openCreateModal').on('click', function() {
        form[0].reset();
        modal.find('.modal-title').text('Create New Master Server');
        form.attr('action', '{{ route('dinetkan.monitoring.server.store') }}');
      ...
      $(document).on('click', '.edit-icon.edit', function() {
        ...
        $.ajax({
          url: `/dinetkan/monitoring/server/single/${userId}`,
          method: 'GET',
          success: function(data) {
            modal.find('.modal-title').text('Update Server');
            form.attr('action', `/dinetkan/monitoring/server/update/${userId}`);
            methodField.val('put');
      ...
      $(document).on('click', '.edit-icon.delete', function() {
        ...
        $.ajax({
          ...
          method: 'GET',
          success: function(data) {
            modal.find('.modal-title').text('Delete Server');
            form.attr('action', `/dinetkan/monitoring/server/delete/${userId}`);
