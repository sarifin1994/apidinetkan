...
<div class="modal fade" id="filterTransaksiModal" tabindex="-1" aria-labelledby="filterTransaksiLabel" aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">
        ...
        <form id="filterTransaksiForm">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        <label for="periode_filter" class="form-label">Periode Transaksi</label>
                        <input type="month" class="form-control" id="periode_filter" name="periode" required>
                    </div>
                    <div class="mb-3">
                        <label for="kategori_filter" class="form-label">Kategori</label>
                        <select class="form-select" id="kategori_filter" name="kategori" required>
                    ...
                    <div class="mb-3">
                        <label for="reseller_filter" class="form-label">Reseller</label>
                        <input type="text" class="form-control" id="reseller_filter" name="reseller" placeholder="Opsional">
                    </div>
                </div>
...
<div class="container-lg">
    ...
    @if (multi_auth()->shortname === 'lintasdb')
        <hr>
        <div class="row p-5 mx-10">
            <span class="text-center fs-5">Data Pemasukan Hotspot Berdasarkan NAS</span>
        </div>
        <div class="row">
            <canvas id="incomeHotspot" width="400" height="200"></canvas>
        </div>
</div>
@endif
@endsection
...
@push('scripts')
<script>
    // Tampilkan modal saat halaman pertama kali dibuka
$(document).ready(function() {
    $('#filterTransaksiModal').modal('show');
});

// Tangani submit form filter
$('#filterTransaksiForm').on('submit', function(e) {
    e.preventDefault();

    const periode = $('#periode_filter').val();
    const kategori = $('#kategori_filter').val();
    const reseller = $('#reseller_filter').val();

    // Simpan filter ke localStorage agar tidak perlu isi ulang saat reload
    localStorage.setItem('filter_periode', periode);
    localStorage.setItem('filter_kategori', kategori);
    localStorage.setItem('filter_reseller', reseller);

    // Kirim parameter filter ke DataTable (bisa disesuaikan ke backend juga)
    table.ajax.reload();
    $('#filterTransaksiModal').modal('hide');
});

    // Ambil data dari controller untuk chart
    const labels = @json($dataBulan);
    const dataNas = @json($dataNas);
    const datasets = [];
    const colors = [
        'rgba(75, 192, 192, 0.2)',
        'rgba(255, 99, 132, 0.2)',
    ...
    let index = 0;
    for (const nasName in dataNas) {
        datasets.push({
            label: nasName,
            data: dataNas[nasName],
    ...
    const myChart = new Chart(
        document.getElementById('incomeHotspot'),
        config
    );
</script>
