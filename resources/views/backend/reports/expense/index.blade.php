...
@section('title', 'Expense Report Management')
        ...
        <div class="card">
          <div class="card-body">
            <div class="table-responsive custom-scrollbar">
              ...
              @include('reports.expense.modal.create')
              @include('reports.expense.modal.edit')
            </div>
          </div>
        </div>
      </div>
...
@section('scripts')
  ...
  <script type="text/javascript">
    const transactionUrl = baseUrl.clone().base() + '/reports/transaction';
    ...
    $('#store_expense').click(function(e) {
      ...
      var data = {
        ...
        'type': $('#type_ce').val(),
        'category': $('#category_ce').val(),
        'item': $('#category_ce').val(),
    ...
    $('#update').click(function(e) {
      ...
      // ajax proses
      $.ajax({
        url: `${transactionUrl}/${transaction_id}`,
        type: "PUT",
        cache: false,
        data: data,
        dataType: "json",
    ...
    $('#myTable').on('click', '#btn-edit-expense', function() {
        ...
        $.ajax({
          ...
          success: function(data) {
            ...
            if (data.data.type === 1 || data.data.type === 2 || data.data.type === 3) {
              $("#type option:selected").val(data.data.type).text('Income');
            } else {
              $("#type option:selected").val(data.data.type).text('Expense');
            }
